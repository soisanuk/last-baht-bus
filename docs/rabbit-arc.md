...
| **2** | The heist quest — fork-not-filter, the inversion, CCIB | Tier 0 | **SHIPPED** — interview (CARRY IT / KEYBOARD / THE KID / NOT ME / ASK), mule + operator + kid paths, the burner, the CCIB landing + radar + BREAK SIM, Tan's read, the dog at the landing |
...
| built | **Laying low has teeth** (Mario: "what happens if they don't?" → "ok, do the build", 2026-09-06): `G.ccibLoud` counts LOUD acts inside the window, one per kind per night (`G.ccibLoudNight`) — going back down Kitten Corner's corridor, walking into Eastern Seaboard after the heist, ASKing anybody who isn't Tan/Eddy/Nont about the police/heist/box/company/coffee, an incident (`_kickOut`, any `_repHit`), a money move ≥ `CCIB_LOUD_MONEY` (฿50k) through Nont's CASH or a wire. Each prints a dim `(… Loud.)` notice — the player is told what counted, never threatened. At `CCIB_LOUD_COFFEE` (2) `_ccibCoffee` fires on the tick at YOUR bar (or the Rabbit's rail, Nuan pouring): the same man, one sentence — *"You were told nothing… and you did something anyway. A footnote that moves is a paragraph."* At `CCIB_LOUD_LAND` (4) `_ccibLand`: syndicate −1 (the one standing that moves — you stopped being inside), Eddy's door shut for the rest of the window (`eddyBackDay`), rep −2 (direct, not via `_repHit`, which would count itself), and Tan's `ccibLanded` read — *"I told you to be boring… A loud man I can only watch."* Never jail, never a visa; nothing further counts once landed. Being boring earns nothing — it is a non-deed — except that none of this happens. Insiders' own questions are never loud (asking Tan is the sanctioned door). Pinned in rabbit.test.js. |
