...
- `web/js/news-data.js` — **auto-generated** (`scripts/fetch-news.mjs`, re-baked every 6 h by `.github/workflows/news.yml` from Google News RSS + Frankfurter/er-api FX; the bake runs the test suite before committing). Classic-script globals `NEWS_FEED` and `FX_RATES` (THB per USD/AUD/GBP/EUR) so no runtime fetch is needed — they surface via WATCH TV / READ PAPER and patron chatter (the FX moan invents a golden-age rate at current × 1.25), and the engine degrades to canned fallbacks when the globals are absent (which is how the vm tests run). **Never gate game logic on headlines or rates** — they change every bake; determinism holds only within a deploy.
