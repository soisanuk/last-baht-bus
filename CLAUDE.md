...
- `web/js/news-data.js` — **auto-generated** (`scripts/fetch-news.mjs`, re-baked every 6 h by `.github/workflows/news.yml` from Google News RSS; the bake runs the test suite before committing). A classic-script global `NEWS_FEED` so no runtime fetch is needed — headlines surface via WATCH TV / READ PAPER and occasional patron chatter, and the engine degrades to canned fallbacks when the global is absent (which is how the vm tests run). **Never gate game logic on headlines** — they change every bake; determinism holds only within a deploy.
