...
**The game is a sandbox.** The wallet quest is Act One: `_checkAct1` fires once at `hotel_room` (gated in `_doGo` on `hasWallet` — the key card is in the wallet), scores it, and converts the score into happiness. `G.over` is never set; the long-game stat is `G.happy` (สนุก), fed by `_addHappy(n)` hooks across every system (social tiers, games via `_endGame`, bell, mama treat, patron chats, encounters, dog bites, ejections, beers with a >4 penalty). Levels at 0/10/25/50/100; 100 = สบายสบาย, celebrated once via the `sabaiSabai` flag but non-terminal. New systems should feed the meter.
